...
class UserController extends Controller
{
    public function indexAction(Request $request)
    {
        ...
        $dql = "SELECT u FROM CRVAUserBundle:User u ORDER BY u.id DESC";
        ...
        $deleteFormAjax = $this->createCustomForm(':USER_ID', 'DELETE', 'crva_user_delete');
    ...
    {
        $em = $this->getDoctrine()->getManager();
        $user = $em->getRepository('CRVAUserBundle:User')->find($id);
        ...
        return $this->render('CRVAUserBundle:User:edit.html.twig', array('user'=>$user, 'form'=>$form->createView()));
    }

    private function createEditForm(User $entity)
    {
        $form = $this->createForm(new UserType(), $entity, array('action' => $this->generateUrl
            ('crva_user_update', array('id' => $entity->getId())), 'method'=>'PUT'));
    ...
    public function updateAction($id, Request $request)
    {
        ...
        if($form->isSubmitted() & $form->isValid()){
            ...
            return $this->redirectToRoute('crva_user_edit', array('id'=> $user->getId()));
        }
    ...
    public function viewAction($id)
    {
        ...
        $deleteForm = $this->createCustomForm($user->getId(), 'DELETE', 'crva_user_delete' );

        return $this->render('CRVAUserBundle:User:view.html.twig', array('user' => $user, 'delete_form' => $deleteForm->createView()));

    }

    //public function createDeleteForm($user)
    //{
    //    return $this->createFormBuilder()
    //        ->setAction($this->generateUrl('crva_user_delete', array('id' => $user->getId())))
    //        ->setMethod('DELETE')
    //        ->getForm();
    //    
    //}
    public function deleteAction(Request $request, $id)
    {
        ...
        $form = $this->createCustomForm($user->getId(), 'DELETE', 'crva_user_delete');
        $form->handleRequest($request);
    ...
    private function deleteUser($role, $em, $user)
    {
        if($role == 'ROLE_USER'){
            ...
            $message = $this->get('translator')->trans('The user cold not be deleted.');
            $remove = 0;
            $alert = 'error';
        }
